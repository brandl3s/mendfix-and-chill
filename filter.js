 ...
 * var completeList = [
 * {
 *  name: "Mending Jumpers",
 *  fix: clothes,
 *  when: 09/04/2017
 * },
 * {
 *  name: "Fix Toasters and Kettles",
 *  fix: electronics,
 *  when: 09/05/2017,
 * }
 * ];
 *
 * var selectedOption = "mendOther";
 *
 * var criteria = [mendClothes,repairElectronics,fixBikes];
 *
...
function filterAndSortList(completeList, selectedOption, criteria) {
  ...
  // Using Array method filter function to iterate through and store the matched items into the filteredList
  ...
  var filteredList = completeList.filter(function(person) {
    ...
    //if the value of 'fix' is 'mendClothes' do this
    if(selectedOption == person.fix){
      console.log(person);
      return true;
  ...
  // Using Array method sort function to iterate through and store the matched items into sortedList
